...
int main(int argc, char **argv) {
    ...
    //ICalErrorCode wasReturned = createCalendar("testCatSimpleUTC.ics", &myCal);
    ...
    if(tempEv1 != NULL) {
        printf("\nshould not occur: tempEv1 from created JSON being printed:\n\nUID = '%s'\n", tempEv1->UID);
        deleteEvent(tempEv1);
    } else {
        printf("\nshould occur: tempEv1 from created JSON was null\n");
    }

    Event *tempEv2 = JSONtoEvent("{\"UID\":\"a\"}");
    if(tempEv2 != NULL) {
        printf("\nshould not occur: tempEv2 from created JSON being printed:\n\nUID = '%s'\n", tempEv2->UID);
        deleteEvent(tempEv2);
    } else {
        printf("\nshould occur: tempEv2 from created JSON was null\n");
    }

    Event *tempEv3 = JSONtoEvent("{\"UID\":\"uid1@exa\b\f\r\n\t\\\"mple.com\"}");
    if(tempEv3 != NULL) {
        printf("\nshould not occur: tempEv3 from created JSON being printed:\n\nUID = '%s'\n", tempEv3->UID);
        deleteEvent(tempEv3);
    } else {
        printf("\nshould occur: tempEv3 from created JSON was null\n");
    }
    ...
    // property list + JSON testing
    char* propListAsJSON = propertyListToJSON(myCal->properties);
    printf("\npropListAsJSON after adding events:\n%s\n", propListAsJSON);
    free(propListAsJSON);

    // alarm list + JSON testing (with added events --done on each event in the iCal)
    char *alarmListAsJSON = NULL;
    int counter = 0;
    evPropIter = createIterator(myCal->events);
	while( (ev = nextElement(&evPropIter)) != NULL ) {
        propListAsJSON = propertyListToJSON(ev->properties);
        printf("\ni=%d: propListAsJSON #%d after adding events:\n%s\n", counter, counter+1, propListAsJSON);
        free(propListAsJSON);
        alarmListAsJSON = alarmListToJSON(ev->alarms);
        printf("\ni=%d: alarmListAsJSON #%d after adding events:\n%s\n", counter, counter+1, alarmListAsJSON);
        free(alarmListAsJSON);
        counter++;
    }
    ...
    Calendar *tempCal01 = JSONtoCalendar("{\"version\":3.456,\"prodID\":\"proIDval\"}");
    if(tempCal01 != NULL) {
        printf("\ntesting JSONtoCalendar #1: tempCal01 from created JSON being printed:\n\nversion = %f   |   prodID = '%s'\n", tempCal01->version, tempCal01->prodID);
        deleteCalendar(tempCal01);
    } else {
        printf("\ntesting JSONtoCalendar #1: tempCal01 from created JSON was null\n");
    }

    Event *tempEv = JSONtoEvent("{\"UID\":1}");
    if(tempEv != NULL) {
        printf("\nshould not occur: testing JSONtoEvent #1: tempEv from created JSON being printed:\n\nUID = '%s'\n", tempEv->UID);
        deleteEvent(tempEv);
    } else {
        printf("\nshould occur: testing JSONtoEvent #1: tempEv from created JSON was null\n");
    }

    // ****************************************************** Testing the new JSON to event ******************************************************

    Event *a3tempEv1 = JSONtoEvent("{\"UID\":\"test value\",\"DTSTAMP\":\"19990103T103012Z\",\"DTSTART\":\"20000204T201520\",\"includeSUMMARY\":true,\"SUMMARY\":\"test sumVal\"}");
    if(a3tempEv1 != NULL) {
        char *toPrint = printEvent((Event*)a3tempEv1);
        printf("\ntesting (a3-modified) JSONtoEvent #1: a3tempEv1 from created JSON being printed:\n\n'%s'\n", toPrint);
        free(toPrint);
        deleteEvent(a3tempEv1);
    } else {
        printf("\ntesting (a3-modified) JSONtoEvent #1: a3tempEv1 from created JSON was null\n");
    }

    Event *a3tempEv2 = JSONtoEvent("{\"UID\":\"test value 2\",\"DTSTAMP\":\"19990103T103012\",\"DTSTART\":\"20000204T201520Z\",\"includeSUMMARY\":false,\"SUMMARY\":\"test sumVal 2\"}");
    if(a3tempEv2 != NULL) {
        char *toPrint = printEvent((Event*)a3tempEv2);
        printf("\ntesting (a3-modified) JSONtoEvent #2: a3tempEv2 from created JSON being printed:\n\n'%s'\n", toPrint);
        free(toPrint);
        deleteEvent(a3tempEv2);
    } else {
        printf("\ntesting (a3-modified) JSONtoEvent #2: a3tempEv2 from created JSON was null\n");
    }
