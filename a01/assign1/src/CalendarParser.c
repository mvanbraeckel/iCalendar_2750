		...
		if(lines[i][0] == ';') { // ------------------------------------ ignore comments ---------------------------------------
			...
			// check END: legal = VCALENDAR
				...
				if(strcmpic(lines[i]+1+3, "VCALENDAR") == 0) {
					...
					endCal = true;
					// check nothing was missing
					if(!VERSION || !PRODID || myCal->events->length == 0) {
						freeParserTemp(currEvent, currAlarm, lines, numLines, myCal, currEventAdded, currAlarmAdded);
						return INV_CAL;
					}
					// reset flags
...
ICalErrorCode writeCalendar(char* fileName, const Calendar* obj) {
	// default return value, because don't need to implement in A1
	return OK;
}
...
int compareEvents(const void* first, const void* second) {
	...
	if(strcmpic(ev1->startDateTime.date, ev2->startDateTime.date) == 0) {
		if(strcmpic(ev1->startDateTime.time, ev2->startDateTime.time) == 0) {
			if(strcmpic(ev1->creationDateTime.date, ev2->creationDateTime.date) == 0) {
				return strcmpic(ev1->creationDateTime.time, ev2->creationDateTime.time);
			} else {
				return strcmpic(ev1->creationDateTime.date, ev2->creationDateTime.date);
			}
		} else {
			return strcmpic(ev1->startDateTime.time, ev2->startDateTime.time);
		}
	} else {
		return strcmpic(ev1->startDateTime.date, ev2->startDateTime.date);
	}
}
