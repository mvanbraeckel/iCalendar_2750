...
bool _tAlarmEqual(const Alarm* testAlarm, const Alarm* refAlarm){
    ...
    //Triger cannot be null
    if (testAlarm->trigger == NULL || refAlarm->trigger == NULL){
        return false;
    }
    
    //Compare trigger
    if(!_tStrEqualIgnCase(testAlarm->trigger, refAlarm->trigger)){
...
bool _tEventEqual(const Event* testEvent, const Event* refEvent){
    ...
    if (!_tDTEqual(testEvent->startDateTime, refEvent->startDateTime)){
