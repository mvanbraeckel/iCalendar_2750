...
char* eventListToJSON(const List* eventList) {
	...
	while( (ev = nextElement(&evIter)) != NULL ) {
		...
		evStr = eventToJSON(ev);	// need to free
...
char* calendarToJSON(const Calendar* cal) {
	...
	sprintf(toReturn, "{\"version\":%.0f,\"prodID\":\"%s\",\"numProps\":%d,\"numEvents\":%d}", verVal, prodIDVal, propVal, evtVal);
	toReturn[strlen(toReturn)] = '\0';
...
Calendar* JSONtoCalendar(const char* str) {
	...
	while( i < len && str[i] != '\"') {
		...
		if(str[i] == '\\') {
			/*// next char must be (b,f,n,r,t) or \ or "
			if(str[i+1] == 'b') {
				pidStr[j++] = '\b';
				i += 2;
				continue;
				...
				// next two chars need to be \n, followed by SPACE or HTAB, then something else 
				if( i+3 > len || str[i+2] != '\\' || str[i+3] != 'n' ) {
					if( (i+6 > len || (str[i+4] != SPACE && (str[i+4] != HTAB || (str[i+4] != '\\' || str[i+5] != 't')))) ) {

					}
					if( (i+6 > len || (str[i+4] != SPACE && (str[i+4] != '\\' || str[i+5] != 't'))) ) {
				...
				// next 4 chars must be hex chars -- currently not making a unicode point char from it
				if(i+5 < len) {
					for(int k = 1; k < 5; k++) {
						if(!ishex(str[i+1+k])) {
							return NULL;
